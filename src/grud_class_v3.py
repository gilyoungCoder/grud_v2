...
# from torch.utils_v1.tensorboard import SummaryWriter
...
def main(learning_rate, learning_rate_decay, n_epochs, n_layers, dropout, weight_decay):
    ...
    if os.path.exists("./input1/x_mean_aft_nor.npy") and os.path.exists("./input1/dataset.npy"):
        pass
    else:
        dataset = np.zeros((1,4, input_length, size))
        ...
        for filename in filenames:
            df = pd.read_csv(inputpath + filename,\
                            header=0,\
                            parse_dates=['Time'],\
                            date_parser=utils_v1.timeparser)
            s_dataset, all_x, id = utils_v1.df_to_x_m_d(df=df, inputdict=inputdict, size=size, id_posistion=id_posistion, split=input_length)
        ...
        x_mean = utils_v1.get_mean(train_x)
        x_std = utils_v1.get_std(train_x)

        x_mean = np.asarray(x_mean)
        x_std = np.asarray(x_std)

        dataset = utils_v1.dataset_normalize(dataset=dataset, mean=x_mean, std=x_std)

        nor_mean, nor_median, nor_std, nor_var = utils_v1.normalize_chk(dataset)
        x_mean = nor_mean
        np.save('./input1/x_mean_aft_nor', nor_mean)
        # np.save('./input/x_median_aft_nor', nor_median)
        np.save('./input1/dataset', dataset)


    A_outcomes = pd.read_csv('./data/physionet/PhysioNet/raw/Outcomes-a.txt')
    # y1_outcomes = utils_v1.df_to_y1(A_outcomes)
    # np.save('./input/y1_out', y1_outcomes)

#    t_dataset = dataset
    t_dataset = np.load('./input1/dataset.npy')
    x_mean = torch.Tensor(np.load('./input1/x_mean_aft_nor.npy'))

    # t_out = np.load('./input/y1_out.npy')
    t_out = utils_v1.df_to_y1(A_outcomes)

    train_dataloader, dev_dataloader, test_dataloader = utils_v1.data_dataloader(t_dataset, t_out, train_proportion=0.8, dev_proportion=0.2)
