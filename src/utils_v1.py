...
def df_to_x_m_d(df, inputdict, size, id_posistion, split):
    ...
    # last_observed_x = np.zeros((len(inputdict)-2, grouped_data.ngroups))
    timetable = np.zeros(grouped_data.ngroups)
    id = 0
    
    all_x = np.zeros((split,1))
    
    s_dataset = np.zeros((4, split, size))
    ...
    if grouped_data.ngroups > size:
        ...
        #     for j in range(grouped_data.ngroups):
        #         if masking[i, j] == 1:
        #             last_observed_x[i, j] = x[i, j]
        #         elif j > 0:
        #             last_observed_x[i, j] = last_observed_x[i, j - 1]
        ...
        last_observed_x = np.zeros((split, size))
